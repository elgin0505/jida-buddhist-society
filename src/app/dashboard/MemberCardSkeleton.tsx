...
export function MemberCardSkeleton() {
  return (
    <div className="w-full">
      {/* 3D 会员卡骨架屏 */}
      <div className="mb-8 will-change-transform transform-gpu" style={{ transform: "translateZ(0)" }}>
        <div className="relative p-6 sm:p-8 overflow-hidden rounded-[22px] bg-[#FAF8F5] md:bg-gradient-to-br md:from-amber-50/60 md:via-warm-cream/70 md:to-amber-100/40 border border-amber-300/40 shadow-lg">
          {/* 金色脉冲微光呼吸层 (仅在桌面端开启) */}
          <div className="pointer-events-none absolute -inset-10 rounded-full bg-[radial-gradient(ellipse_at_top_right,rgba(251,191,36,0.35)_0%,rgba(245,158,11,0.15)_45%,transparent_70%)] blur-2xl animate-pulse hidden md:block" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="h-24 rounded-2xl bg-[#FAF8F5] dark:bg-slate-800 md:bg-white/50 md:dark:bg-slate-800/50 md:backdrop-blur-xs border border-ocher/10 animate-pulse p-4 flex flex-col justify-between"
          >
      ...
      <div className="mb-6 flex justify-center">
        <div className="inline-flex rounded-2xl bg-[#FAF8F5] dark:bg-slate-800 md:bg-white/70 md:dark:bg-slate-800/80 p-1.5 shadow-sm border border-ocher/20 dark:border-white/10 md:backdrop-blur-md gap-2">
