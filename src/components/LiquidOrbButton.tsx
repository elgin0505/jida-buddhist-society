...
// Module-level WebGPU singleton cache to prevent multiple GPUDevice instances
// which causes "WebGPU rendering error: device mismatch" in React 19 / Fast Refresh
let sharedDevice: any = null;
let sharedDevicePromise: Promise<any> | null = null;
let sharedResources: {
  device: any;
  format: any;
  pipeline: any;
  ribbonPipeline: any;
  ribbonCompositePipeline: any;
  ribbonSampler: any;
} | null = null;

async function getSharedLiquidOrbResources(shaderSource: string) {
  if (sharedResources && sharedDevice) {
  ...
  sharedDevicePromise = (async () => {
    ...
    device.lost.then((info: any) => {
      console.warn("[LiquidOrb] WebGPU device lost:", info?.message || info?.reason);
      sharedDevice = null;
    ...
    device.addEventListener("uncapturederror", (event: any) => {
      ...
      console.warn("[LiquidOrb] WebGPU device uncaptured error:", event.error?.message || event.error);
    });
    ...
    if (errors.length) {
      throw new Error(errors.map((message: any) => `${message.lineNum}:${message.linePos} ${message.message}`).join("\n"));
    }
...
export default function LiquidOrbButton({ onClick }: { onClick: () => void }) {
  ...
  useEffect(() => {
    ...
    let ribbonCompositeBindGroup: any = null;
    let state = "thinking";
    ...
    function stopWithError(error: any) {
      ...
      setError(message);
      console.warn("[LiquidOrb] WebGPU fallback activated:", message);
    }
    ...
    async function start() {
      ...
      const res = await getSharedLiquidOrbResources(shaderSource);
      if (!isMounted || stopped) return;

      device = res.device;
      const { format, pipeline, ribbonPipeline, ribbonCompositePipeline, ribbonSampler } = res;

      context = canvas.getContext("webgpu");
      if (!context || !isMounted || stopped) {
      ...
      function frame(now: number) {
        if (stopped || !isMounted || !device) return;
        try {
          ...
          if (stopped || !isMounted || !device || !context) return;
